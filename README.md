...
- operators (op) = CSV file with all operators sand their Keys.
- operators-distribution (opd) = CSV file with the operator distribution per cluster.
...
```bash
yarn cli custom-bulk --keystore=keystore.json --password=test -opd=operators-distribution.csv -op=operators.csv --output-folder=./ --owner-address=... --owner-nonce=..
```
