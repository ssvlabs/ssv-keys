...
async function main() {
  ...
  const ssvKeys1 = new SSVKeys(SSVKeys.VERSION.V3);
  const privateKey = await ssvKeys1.getPrivateKeyFromKeystoreData(keystore, keystorePassword);

  // 2. At some point we get operator IDs and public keys and want to save them too
  await ssvKeys1.keySharesInstance.setData({
  ...
  await fsp.writeFile(getKeySharesFilePath(2), ssvKeys1.keySharesInstance.toString(), { encoding: 'utf-8' });

  // 4. Build shares from operator IDs and public keys
  const shares = await ssvKeys1.buildShares(privateKey, operatorIds, operators);

  // Now save to key shares file encrypted shares and validator public key
  await ssvKeys1.keySharesInstance.setData({
    publicKey: ssvKeys1.getValidatorPublicKey(),
    shares,
  });

  await fsp.writeFile(getKeySharesFilePath(3), ssvKeys1.keySharesInstance.toString(), { encoding: 'utf-8' });

  // Build final web3 transaction payload and update keyshares file with payload data
  await ssvKeys1.buildPayload(
    ssvKeys1.getValidatorPublicKey(),
    operatorIds,
    shares,
    123456789,
  );
  // await ssvKeys.keySharesInstance.setPayload(payload);
  await fsp.writeFile(getKeySharesFilePath(4), ssvKeys1.keySharesInstance.toString(), { encoding: 'utf-8' });


  const ssvKeys2 = new SSVKeys(SSVKeys.VERSION.V3);
  // Build payload with a new ssv amount and from saved on previous steps key shares data
  const keySharesWithoutPayload = await ssvKeys2.keySharesInstance.init(String(await fsp.readFile(getKeySharesFilePath(3))));
  ssvKeys2.buildPayloadFromKeyShares(keySharesWithoutPayload, 987654321);
  // await ssvKeys.keySharesInstance.setPayload(payload2);

  // Save new key shares file with new ssv amount
  await fsp.writeFile(getKeySharesFilePath(5), ssvKeys2.keySharesInstance.toString(), { encoding: 'utf-8' });
